#from meteo_utils import MeteoDataGenerator
#el cliente tiene que cojer generator MeteoDataGenerator()
#while true: 
# data: generator.analyze_air()/analyze_pollution()
# timestamp : time.time()
# serverConnection.send(data, timestamp)
# time.sleep(x) (x son los segundos que tarda en procesar el servidor)
#data y timestamps se tiene que meter en una clase que te genera el proto 
#esto es a groso modo lo que hace el cliente
#para el servidor en server.py(dentro creamos clase servicer() bque importa los datos (de protobuff compiles files) i la logiva del servidor que es el server_service), server_service.py
#el LB recive los datos en formato protobuff i la propia logica tiene que hacer una llamada grpc cliente de send()
#se puede hacer lo de pretty.install y ir viendo como cambia cada 10 segundos o así
#no hay mucha tolerancia a fallos
...
class ProcessedData:
    def __init__(self, well, poll, timestampWell, timestampPoll):
        self.well = well
        self.poll = poll
